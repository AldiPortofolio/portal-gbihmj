    ...
    <div class="portlet light bordered">
        ...
        <div class="portlet-body form">
            ...
            <div class="row">
                ...
                <div class="col-md-6">
                    <div class="form-group" style=''>
                          <label for="tag">District</label>
                          <select class="select2" name="district">
                            @foreach($district as $u)
                                <option value="{{$u->id}}" {{old('district') ? ($u->id == old('district') ? 'selected' : '') : ($data->district_id ? ($data->district_id == $u->id ? 'selected' : '') : '' )}}>{{$u->name}}</option>
                            @endforeach
                          </select>
                    </div>
                </div>

                {!!view($view_path.'.builder.text',['type' => 'text','name' => 'name','label' => 'Subdistrict Name','value' => (old('name') ? old('name') : $data->name),'attribute' => 'required autofocus','form_class' => 'col-md-6', 'class' => ''])!!}
