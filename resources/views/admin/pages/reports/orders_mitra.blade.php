      ...
      <form role="form" class="form-horizontal">
        <div class="form-body">
          <div class="row">
            ...
            <div class="col-md-6">
              <div class="form-group form-md-line-input">
                <label class="col-md-4 control-label" for="form_control_1">Mitra</label>
                <div class="col-md-8">
                    <select id="mitra" class="form-control select2-multiple" multiple>
                      <optgroup label="Mitra">
                        @foreach($mitra as $u)
                          <option value="mitra-{{$u->id}}">{{$u->name}}</option>
                        @endforeach
                      </optgroup>
                  </select>
                    <div class="form-control-focus"> </div>
  ...
  <script>
    jQuery(document).ready(function() {
      ...
      $("#search").on("click", function() {
          var searchMitra = $("#mitra").select2('data');
          ...
          for(var i in searchMitra){
              var getOption = searchMitra[i].id.split("-");
              var option = getOption[0];

              mitraId.push(getOption[getOption.length - 1]);
          }

          $.ajax({
              type: "POST",
              url: $.cur_url() + "/ext/filter",
              data: { mitra_id: mitraId, search_date_from: searchDateFrom, search_date_to: searchDateTo, format: searchFormat } ,
          }).done(function(msg) {
