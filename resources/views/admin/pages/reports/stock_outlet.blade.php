      ...
      <form role="form" class="form-horizontal">
        <div class="form-body">
          <div class="row">
            ...
            <div class="col-md-6">
              <div class="form-group form-md-line-input">
                <label class="col-md-4 control-label" for="form_control_1">Product</label>
                <div class="col-md-8">
                    <select id="product" class="form-control select2-multiple" multiple>
                      <optgroup label="Product">
                        <option value="0">-- All Product --</option>
                        @foreach($product as $p)
                          <option value="product-{{$p->id}}">{{$p->product_name}}</option>
                        @endforeach
                      </optgroup>
                  </select>
  ...
  <script>
    jQuery(document).ready(function() {
      ...
      $("#search").on("click", function() {
          var searchProduct = $("#product").select2('data');
          ...
          for(var i in searchProduct){
              var getOption = searchProduct[i].id.split("-");
              var option = getOption[0];

              arr.push(getOption[getOption.length - 1]);
          }

          $.ajax({
              type: "POST",
              url: $.cur_url() + "/ext/filter",
              data: { product_id: arr, search_date_from: searchDateFrom, search_date_to: searchDateTo},
          }).done(function(msg) {
